...
void OpenGLWindow::initializeGL() {
  ...
  // Load default model
  loadModel(getAssetsPath() + "Wolf.obj");
  loadBallModel(getAssetsPath() + "Ball.obj");
  m_mappingMode = 2;

  // Load cubemap
  m_model.loadCubeTexture(getAssetsPath() + "textures/cube/");
...
void OpenGLWindow::loadModel(std::string_view path) {
  m_model.loadDiffuseTexture(getAssetsPath() + "textures/Wolf.png");
  m_model.loadNormalTexture(getAssetsPath() + "textures/WolfNormal.png");
...
void OpenGLWindow::loadBallModel(std::string_view path) {
  ball_model.loadDiffuseTexture(getAssetsPath() + "textures/Ball.png");
  ball_model.loadNormalTexture(getAssetsPath() + "textures/BallNormal.png");
 
  ball_model.loadFromFile(path, true, true);
  
  ball_model.setupVAO(m_programs.at(m_currentProgramIndex));

  ball_trianglesToDraw = ball_model.getNumTriangles();

  // Use material properties from the loaded model
  ball_Ka = ball_model.getKa();
  ball_Kd = ball_model.getKd();
  ball_Ks = ball_model.getKs();
  ball_shininess = ball_model.getShininess();
}
...
void OpenGLWindow::paintGL() {
  ...
  glUniform1f(shininessLoc, ball_shininess);
  glUniform4fv(KaLoc, 1, &ball_Ka.x);
  glUniform4fv(KdLoc, 1, &ball_Kd.x);
  glUniform4fv(KsLoc, 1, &ball_Ks.x);

  m_model.render(m_trianglesToDraw);
  ball_model.render(ball_trianglesToDraw);
}
...
void OpenGLWindow::paintUI() {
  ...
  // File browser for normal textures
...
// Only in WebGL
  ...
  fileDialogDiffuseMap.SetPwd(getAssetsPath() + "/textures");
  fileDialogNormalMap.SetPwd(getAssetsPath() + "/textures");
#endif
  ...
  // Create main window widget
  {
...

    {
      ...
      std::vector<std::string> comboItems{"Perto", "Longe"};

      ImGui::PushItemWidth(120);
      if (ImGui::BeginCombo("Zoom",
  ...
  if (fileDialogDiffuseMap.HasSelected()) {
    ...
    ball_model.loadDiffuseTexture(fileDialogDiffuseMap.GetSelected().string());
  ...
  if (fileDialogNormalMap.HasSelected()) {
    ...
    ball_model.loadNormalTexture(fileDialogNormalMap.GetSelected().string());
